...
func KnapsackDynamicWeight(capacity int, items []FItem) (float64, []int) {

	var matrix = make([][]float64, capacity + 1)
	for i := 0; i <= capacity; i++ {
		matrix[i] = make([]float64, len(items) + 1)
	}
	...
	for w := 0; w <= capacity; w++ {
		...
		for i := 0; i <= len(items); i++ {
			...
			if w == 0 || i == 0 {
				...
				right := matrix[w][i - 1]
				matrix[w][i] = maxF(left, right)
			}
		}
	}

	var config []int
	config = backtrackF(items, capacity, matrix, capacity, len(items), config)
...
func backtrackF(items []FItem, capacity int, matrix [][]float64, indexWeight int, indexItem int, config []int) []int {
	if indexItem == 0 {
		return config
	}

	if items[indexItem - 1].Weight == 0 || matrix[indexWeight][indexItem] != matrix[indexWeight][indexItem - 1] {
		config = backtrackF(items, capacity, matrix, indexWeight - items[indexItem - 1].Weight, indexItem - 1, config)
		config = append(config, 1)
	} else {
		config = backtrackF(items, capacity, matrix, indexWeight, indexItem - 1, config)
		config = append(config, 0)
	}
	return config
}
